...
MHW_daily_distance_df <- MHW_daily_distance_df %>%
  group_by(Serial,fish_id,Serial_fish_id, solar_date) %>%
  filter(!is.na(distance_shore)) %>%
  summarise(
    n = n(),
    # Max daily displacement: median of the top 10% - median of the bottom 10%
  ...
  ungroup()%>%
  distinct(Serial,fish_id,Serial_fish_id, solar_date, .keep_all = TRUE)  # Remove duplicates explicitly
...
sum_heat_wave_disp <- sum_heat_wave %>%
  filter(!is.na(mean_disp_max) & window_factor==1.5 ) %>% 
  select(fish_id,Serial,Before_After,mean_disp_max,sd_disp_max,disp_max_days,species) %>% distinct()


# Plot
disp_symbol_compare_plot <- ggplot(sum_heat_wave_disp, aes(x = Before_After, y = mean_disp_max)) +
  geom_boxplot(aes(fill = Before_After),alpha = 0.7,show.legend = FALSE) +
  scale_fill_manual(values = c("Before" = "#2E8B57",
                               "MHW" = "#DC143C",
                               "After" = "#4169E1"))+
...
ggsave("Red Sea/Results/Plots/act_dep_disp_compare_plot_OISST_Fix.svg", full_plot, width = 12, height = 10)
