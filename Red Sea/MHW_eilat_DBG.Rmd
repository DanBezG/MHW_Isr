...
```{r}
MHWs_stat_lm <- MHWs_stat %>% filter(window_factor==1.5)

hist(MHWs_stat_lm$ln_activity_ratio)
lm_act<- lm(ln_activity_ratio ~ intensity_cumulative*Stage_period,data = MHWs_stat_lm)
...
n_df_act <- MHWs_stat_lm %>%
  group_by(Stage_period, intensity_cumulative) %>%
  filter(!is.na(ln_activity_ratio)) %>% 
  summarise(n = n(),
            mean_ln = mean(ln_activity_ratio, na.rm = TRUE),
            .groups = "drop") 

ggplot(MHWs_stat_lm, aes(x = intensity_cumulative, y = ln_activity_ratio)) +
  ...
  geom_text(data = n_df_act, aes(x = intensity_cumulative, y=-Inf ,angle = 90, label = n),
            inherit.aes = FALSE, size = 3, fontface = "bold",hjust=-2) +
...
hist(MHWs_stat_lm$delta_depth)

lm_dep<- lm(delta_depth ~ intensity_cumulative*Stage_period,data = MHWs_stat_lm)
...
ggplot(MHWs_stat_lm, aes(x = intensity_cumulative, y = delta_depth)) +
  stat_summary(fun = mean, geom = "point", size = 2, color = "black") +
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.2, color = "black") +
  geom_smooth(method = "lm", se = TRUE, color = "darkred", linewidth = 1) +
  stat_poly_eq(
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  geom_text(data = n_df_dep, aes(x = intensity_cumulative, y=-Inf ,angle = 90, label = n),
            inherit.aes = FALSE, size = 3, fontface = "bold",hjust=-2) +
  facet_wrap(~ Stage_period) +
  labs(
    x = "Cumulative Intensity",
...
##max daily distance
ggplot(MHW_daily_distance_df,aes(x=Before_After,y=max_daily_displacement))+
  geom_boxplot()+
  geom_point()+
  theme_bw()
...
MHW_daily_distance_df <- MHW_daily_distance_df %>%
  mutate(Before_After = factor(Before_After, levels = c("Before", "MHW", "After")))

mean_disp_df <- MHW_daily_distance_df %>%
  group_by(Serial_fish_id, Before_After) %>%
  summarise(mean_disp = mean(max_daily_displacement, na.rm = TRUE)) %>%
  ungroup()
