...
run_model_safely <- function(formula, data, family) {
  model <- tryCatch(
    ...
    warning = function(w) {
      message("⚠️ Warning: ", conditionMessage(w))
...
automate_model_selection <- function(data, response_var, family) {
  ...
  # -------------------------------------------
  # 2️⃣ Step 2: Add MHW effect (smooth or fixed)
  # -------------------------------------------
  ...
  message("🏆 Best model with MHW: ", deparse(formula(best_with_MHW)))
  
  # -------------------------------------------
  # 3️⃣ Step 3: Add species effect
  # -------------------------------------------
  model_formulas_with_species <- list(
    formula(best_with_MHW),
    update.formula(formula(models_with_MHW[[1]]), . ~ . + species)
  )
  ...
  message("🏆 Best final model with species: ", deparse(formula(best_final)))
...
MHWs_Eilat_Moving_OISST <- read.xlsx("Red Sea/MHWs_def/MHW__detrend_31mm_OISST_Events_RedSea.xlsx",sheet = "Short_MHW_Events_Fixed")
...
for (event in 1:dim(MHWs_Eilat)[1]) {
  ...
  if(length(na.omit(fish_ids))>0)
  {
    ...
    MHW_Data <- filter_fish %>% filter(date>= MHWs_Eilat$start_3[event] & date<= 
                                              MHWs_Eilat$end_3[event])
...
saveRDS(results_depths,"Red Sea/Results/Models/Depth_Models_OISST_detrended_31mm.RDS")
...
saveRDS(results_activity,"Red Sea/Results/Models/Act_Models_OISST_detrended_31mm.RDS")
