...
models_comparison <- function(models)
{
  ...
  # Compute ΔR² (relative to best model)
...
run_model_safely <- function(formula, data, family) {
  model <- tryCatch(
    ...
    error = function(e) {
      message("❌ Model failed to run: ", conditionMessage(e))
      return(NULL)  # Return NULL for failed models
    },
    warning = function(w) {
      message("⚠️ Warning: ", conditionMessage(w), "Model: ", formula)
  ...
  if (!is.null(model) && !model$converged) {
    message("⚠️ Model did not converge and will be removed.")
    return(NULL)
  }
...
#    1️⃣ Fit models without MHW effect
#    2️⃣ Add MHW effect (smooth or fixed)
#    3️⃣ Add species effect
# Returns: 
#    ✅ The best final model
#    ✅ A list of all models created along the way
# -------------------------------
automate_model_selection <- function(data, response_var, family) {
  ...
  # -------------------------------------------
  # 1️⃣ Step 1: Fit initial models (without MHW effect)
  # -------------------------------------------
  ...
  if (length(models_no_MHW) == 0) stop("❌ No models converged in the first step!")
  ...
  message("🏆 Best model without MHW: ", deparse(formula(best_no_MHW)))
  ...
  #   message("⚠️ No models converged with species effect. Returning best model without species.")
  ...
  # message("🏆 Best model with species: ", deparse(formula(best_with_species)))
  ...
  if (length(models_with_MHW) == 0) stop("❌ No models converged with MHW effect!")
  ...
  message("🏆 Best final model with MHW: ", deparse(formula(best_final)))
...
MHWs_Eilat_Moving_OISST <- read.xlsx("Red Sea/MHWs_def/MHW_OISST_Events_RedSea_Jacox.xlsx",sheet = "Short_MHW_Events_Fixed")
...
MHWs_Eilat <- MHWs_Eilat_Moving_OISST  # Set the dataset to work with
...
MHWs_stat <- MHWs_stat %>% filter(!(is.na(ln_activity_ratio) & is.na(delta_depth)))
...
############ Choose the name of the basedataset and def #############
write.csv(MHWs_stat,paste("Red Sea/Results/Individual Heatwaves/MHWs_Stat_Amazia_Fixbase_MHW_Stage_Avg_long",Sys.Date(),".csv"))
```
...
# Only MHW:Before
MHWs_stat_lm <- MHWs_stat %>% filter(window_factor==1.5 & Stage_period=="After : MHW" )

## Several stage period
# MHWs_stat_lm <- MHWs_stat %>% filter(window_factor==1.5 & (Stage_period=="MHW : Before" | Stage_period=="After : MHW"))
...
hist(MHWs_stat_lm$ln_activity_ratio)
lm_act<- lm(ln_activity_ratio ~ intensity_cumulative,data = MHWs_stat_lm)
gratia::appraise(lm_act)
summary(lm_act)

# Make sure factor levels are in correct order
MHWs_stat_lm$Stage_period <- factor(MHWs_stat_lm$Stage_period,
                                 levels = c("MHW : Before", "After : Before", "After : MHW"))


# Step 1: Summarize n per group
# Summary stats for label positioning
n_df_act <- MHWs_stat_lm %>%
...
act_reg_plot <- ggplot(MHWs_stat_lm, aes(x = intensity_cumulative, y = ln_activity_ratio)) +
...
sum_heat_wave_act <- sum_heat_wave %>%
  filter(!is.na(mean_activity) & window_factor==1.5 )


# Plot
act_symbol_compare_plot <- ggplot(sum_heat_wave_act, aes(x = Before_After, y = mean_activity)) +
  ...
  geom_jitter(aes(shape = Period),position = position_jitterdodge(jitter.width = 0.15, dodge.width = 0.8),size=2.5)+
    # geom_jitter(aes(shape = Period),size=2.5)+
  # facet_wrap(~species,scales="free_y")+
...
ggsave("Red Sea/Results/Plots/act_symb_compare_plot_Amazia_Fix.svg", act_symbol_compare_plot, width = 12, height = 10)


# Plot - color instead of shapes
color_compare_plot <- ggplot(sum_heat_wave_act, aes(x = Before_After, y = mean_activity)) +
  geom_boxplot(show.legend = FALSE) +
  scale_color_manual(values = c("Dawn" = "#DAA520",
                              "Day" = "#4682B4",
                              "Dusk" = "#BA55D3",
                              "Night" = "#708090")) +
  geom_jitter(aes(color = Period),size=2,shape=17)+
...
act_com_plot <- act_symbol_compare_plot+act_reg_plot + plot_layout(widths = c(2,4))

## Depth

lm_dep<- lm(delta_depth ~ intensity_cumulative,data = MHWs_stat_lm)
gratia::appraise(lm_dep)
summary(lm_dep)

# Step 1: Summarize n per group
# Summary stats for label positioning
n_df_dep <- MHWs_stat_lm %>%
  group_by(intensity_cumulative) %>%
...
dep_reg_plot <- ggplot(MHWs_stat_lm, aes(x = intensity_cumulative, y = delta_depth)) +
  stat_summary(fun = mean, geom = "point", size = 2, color = "black") +
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.2, color = "black") +
  geom_smooth(method = "lm", se = TRUE, color = "darkred", linewidth = 1) +
  stat_poly_eq(
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  geom_text(data = n_df_dep, aes(x = intensity_cumulative, y=Inf , label = n),
            inherit.aes = FALSE, size = 3, fontface = "bold",vjust=2) +
  labs(
    x = "Cumulative Intensity",
    y = "Delta depth [m])"
...
sum_heat_wave_dep <- sum_heat_wave %>%
  filter(!is.na(mean_depth) & window_factor==1.5 )

# Plot
dep_symbol_compare_plot <- ggplot(sum_heat_wave_dep, aes(x = Before_After, y = mean_depth)) +
  geom_boxplot(aes(fill = Before_After),alpha = 0.7,show.legend = FALSE) +
  scale_fill_manual(values = c("Before" = "forestgreen",
                               "MHW" = "red",
                               "After" = "dodgerblue"))+
  ...
  theme_bw() +
  theme(legend.position = "top")
ggsave("Red Sea/Results/Plots/dep_symb_compare_plot_Amazia_Fix.svg", dep_symbol_compare_plot, width = 12, height = 10)

dep_com_plot <- dep_symbol_compare_plot+dep_reg_plot + plot_layout(widths = c(2,4))
ggs
full_plot <- act_com_plot / dep_com_plot

ggsave("Red Sea/Results/Plots/act_dep_compare_plot_OISST_Fix.svg", full_plot, width = 12, height = 10)
...
```{r}
####### choose which dataset to work on through MHWs_Eilat  ###########
MHWs_Eilat <- MHWs_Eilat_Fix_Amazia
...
MHW_daily_distance_df <- MHW_daily_distance_df %>%
  group_by(Serial_fish_id, solar_date) %>%
  ...
  summarise(
      ...
      ifelse(n>50,
             {
              ...
              abs(median(top_10_percent, na.rm = TRUE) - median(bottom_10_percent,
                                                                na.rm = TRUE))         
              }
      ,NA)
    ...
    night_displacement_6 =
      {
      filter_distances <- distance_shore[Period == "Day" | Period == "Dusk" | Period ==
                                           "Dawn"]
  ...
  ungroup()%>%
  distinct(Serial_fish_id, solar_date, .keep_all = TRUE)  # Remove duplicates explicitly
...
hist(MHW_daily_distance_df$night_displacement_6)
hist(MHW_daily_distance_df$night_displacement_12)
hist(MHW_daily_distance_df$night_displacement_18)
long_night_displacement_df <- MHW_daily_distance_df %>%
  pivot_longer(cols = starts_with("night_displacement_"),
               names_to = "displacement_type",
               values_to = "distance")

ggplot(long_night_displacement_df, aes(x = displacement_type, y = distance)) +
  stat_summary(fun = mean, geom = "point", size = 4) +
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.2) +
  labs(x = "Displacement Type", y = "Distance from Shore (m)") +
  theme_minimal()
...
hist(MHW_daily_distance_df$night_displacement_6[MHW_daily_distance_df$Before_After=="Before"])
hist(MHW_daily_distance_df$night_displacement_6[MHW_daily_distance_df$Before_After=="MHW"])
hist(MHW_daily_distance_df$night_displacement_6[MHW_daily_distance_df$Before_After=="After"])



MHW_daily_distance_df$Before_After <- factor(MHW_daily_distance_df$Before_After, levels = c("Before", "MHW", "After"))

# Calculate the mean and CI for each serial_fish_id and Before_After
MHW_disp_summary <- MHW_daily_distance_df %>%
  group_by(Serial_fish_id,species, Before_After) %>%
  summarise(
    mean_disp_max = mean(max_daily_displacement, na.rm = TRUE),
    sd_disp_max = sd(max_daily_displacement, na.rm = TRUE),  # Standard deviation
    n_disp_max = sum(!is.na(max_daily_displacement)),  # Number of non-NA values
...
MHW_disp_delta <- MHW_disp_summary %>%
  pivot_wider(names_from = Before_After, values_from = mean_disp_max) %>%
  mutate(
    delta_MHW_Before = `MHW` - `Before`,  # MHW - Before
    delta_After_Before = `After` - `Before`  # After - Before
  )

mean_disp_df <- MHW_daily_distance_df %>%
  group_by(Serial_fish_id, Before_After) %>%
  summarise(mean_max_disp = mean(max_daily_displacement, na.rm = TRUE),
            mean_night_disp = mean(night_displacement_6, na.rm = TRUE)) %>%
  ungroup()

before_disp_df <- mean_disp_df %>%
  filter(Before_After == "Before") %>%
  select(Serial_fish_id, before_disp = mean_disp)

log_ratios_distances <- mean_disp_df %>%
  left_join(before_disp_df, by = "Serial_fish_id") %>%
  mutate(
    log_ratio = log(mean_disp / before_disp)  # log(MHW/Before) and log(After/Before)
  ) %>%
  filter(Before_After != "Before" & !is.na(log_ratio))  # Remove the "Before" row, as it's the reference

summary_log_ratios_distances <- log_ratios_distances %>%
  group_by(Before_After) %>%
  summarise(
    mean_log_ratio = mean(log_ratio, na.rm = TRUE),
    ci_low = mean_log_ratio - qt(0.975, df = n() - 1) * sd(log_ratio, na.rm = TRUE) / sqrt(n()),
    ci_high = mean_log_ratio + qt(0.975, df = n() - 1) * sd(log_ratio, na.rm = TRUE) / sqrt(n())
  )

ggplot(summary_log_ratios_distances, aes(x = Before_After, y = mean_log_ratio)) +
  geom_point(size = 3) +  # Add points for the mean
  geom_errorbar(aes(ymin = ci_low, ymax = ci_high), width = 0.2) +  # Add error bars for CI
  labs(
    x = "Stage (Before_After)",
    y = "Mean Max Daily Displacement"
  ) +
  theme_minimal()
...
points_per_stage <- hourly_depth_counts %>%
...
simple_model_dep <- bam(log_depth ~ Before_After +s(DecimalTimeOfDay,by = Before_After,k=12,bs="cc") ,data = MHWs_fish_data_dep_filtered,family = "tw",knots = list(c(-0.5,24.5)),method = "REML")
gratia::draw(simple_model_dep)
summary(simple_model_dep)
appraise(simple_model_dep)
...
summary(results_depths$best_model)
gratia::draw(results_depths$best_model)
...
MHWs_fish_data_act_filtered<- df %>%
  filter(Serial_fish_id %in% valid_ids)
